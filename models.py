...
def get_generators(triple_train, triple_val, triple_test, batch_size, prot2embed, option, embed_dict=None):
    if option == "seq":
        generator = seq_Generator(triple_train[:,0:2], triple_train[:,3], batch_size, prot2embed)
        val_generator = seq_Generator(triple_val[:,0:2], triple_val[:,3], batch_size, prot2embed)
        test_generator = seq_Generator(triple_test[:,0:2], triple_test[:,3], batch_size, prot2embed)
    elif option == "joint":
        ...
        generator = go_Generator(triple_train[:,0:3], triple_train[:,3], batch_size, prot2embed, embed_dict)
        val_generator = go_Generator(triple_val[:,0:3], triple_val[:,3], batch_size, prot2embed, embed_dict)
        test_generator = go_Generator(triple_test[:,0:3], triple_test[:,3], batch_size, prot2embed, embed_dict)
...
class seq_Generator(Sequence):
    ...
    def __getitem__(self, idx):
        ...
        X_batch_list1 = np.empty((batch_len, self.MAXLEN,22), dtype=np.float32)
        X_batch_list2 = np.empty((batch_len, self.MAXLEN,22), dtype=np.float32)
...
class go_Generator(Sequence):
    ...
    def __getitem__(self, idx):
        ...
        x_seq1 = np.empty((batch_len, 1000,22), dtype=np.float32)
        x_seq2 = np.empty((batch_len, 1000,22), dtype=np.float32)
        x_pheno = np.empty((batch_len, self.dim_input), dtype=np.float32)
        y_batch = np.empty(batch_len, dtype=np.float32)

        for ids in range(start, min((idx + 1) * self.batch_size, self.length)):
            x_seq1[ids-start,:,:] = self.prot2embed[self.x[ids][0]]
            x_seq2[ids-start,:,:] = self.prot2embed[self.x[ids][1]]
            x_pheno[ids-start,:] = self.embed_dict[self.x[ids][0]]
            y_batch[ids-start] = self.y[ids]
        return [x_seq1, x_seq2, x_pheno], y_batch