...
model_file = f'models/model_half_{option}_{thres}_{tid}.h5'
out_file = f'stats/tune_half_{option}_{thres}_{tid}.pckl'
preds_file = f'preds/preds_half_{option}_{thres}_{tid}.txt'
...
with open(swissprot_file, 'r') as f:
    ...
    for line in f:
        ...
        if items[0] not in embed_dict or len(items[3]) > MAXLEN:
            continue
        hp_set.add(items[0])
...
with open(hpi_file, 'r') as f:
    for line in f:
        ...
        if 'family' not in items[4] or 'miscore:' not in items[5] \
                    or len(items[6]) > MAXLEN or items[0] not in hp_set:
            continue
...
class Generator(Sequence):
    ...
    def __getitem__(self, idx):
        ...
        return [x_seq1, x_pheno1, x_pheno2], y_batch
...
def half_model():
    ...
    # to match shape for dot product; specific for the half model
    dense_seq2 = Dense(params['dense_units'])(flat)
    activ_seq2 = LeakyReLU(alpha=0.1)(dense_seq2)
    dropout_seq2 = Dropout(0.5)(activ_seq2)

    return seq, pheno, dropout_seq2
...
for test_family in filtered_families:
    ...
    seq1, pheno1, flat1 = half_model()
    pheno2, flat2 = pheno_model()
    
    final = Dot(axes=-1, normalize=True)([flat1, flat2])
    output = Dense(1, activation='sigmoid')(final)

    model = Model(inputs=[seq1, pheno1, pheno2], outputs=output)
    model.summary()
    model.compile(
